...
def get_mosei_from_tbje(args):
    ...
    (
        ...
        open(args.data_root + "/data/mosei/tbje_mosei_updated.pkl", "rb"), encoding="latin1"
    )
...
def get_mosei_from_tbje_emotion(args):
    (
        ...
        open(args.data_root + "/data/mosei/tbje_mosei_" + args.emotion + ".pkl", "rb"),
        encoding="latin1",
    )
...
def main(args):
    if args.dataset == "iemocap":
        ...
        cogmen.utils.save_pkl(data, args.data_root + "/data/iemocap/data_iemocap.pkl")
    if args.dataset == "iemocap_4" and args.split_utterances == -1:
        train, dev, test = get_iemocap()
        data = {"train": train, "dev": dev, "test": test}
        cogmen.utils.save_pkl(data, args.data_root + "/data/iemocap_4/data_iemocap_4.pkl")
    if args.dataset == "iemocap_4" and args.split_utterances != -1:
    ...
    if args.dataset == "mosei":
        ...
        data = {"train": train, "dev": dev, "test": test}
        cogmen.utils.save_pkl(data,args.data_root + "/data/mosei/data_mosei.pkl")

    if args.dataset == "mosei_tbje_2class":
        train, dev, test = get_mosei_from_tbje(args)
        data = {"train": train, "dev": dev, "test": test}
        cogmen.utils.save_pkl(data,args.data_root + "/data/mosei/data_mosei_2class.pkl")

    if args.dataset == "mosei_tbje_7class":
        train, dev, test = get_mosei_from_tbje(args)
        data = {"train": train, "dev": dev, "test": test}
        cogmen.utils.save_pkl(data,args.data_root + "/data/mosei/data_mosei_7class.pkl")

    if args.dataset == "mosei_emotion":
        train, dev, test = get_mosei_from_tbje_emotion(args)
        data = {"train": train, "dev": dev, "test": test}
        cogmen.utils.save_pkl(data,args.data_root + "/data/mosei/data_mosei_" + args.emotion + ".pkl")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=".",
        help="data root for google colab",
    )
